...
abstract class Creature {
	...
	//les méthodes 'abstract' ne sont pas définies dans la classe. Elles sont à définir par les classes filles
	...
	public void setLocation(int x, int y) {
		...
		this.move(tmpx, tmpy); //move(1,0) ferait bouger la creature d'une case vers le bas ?
	}
	...
	/** Définition de l'interaction, à un endroit donné, entre un élément Food de la map et une instance Creature
	 * (e.g. une instance de Pacman va "manger" une instance de Food mais une instance de Ghost non) 
	 * 
	 * @param map sur laquelle évolue l'instance Creature
	 * @param i coordonnées de l'instance 
	 * @param j
	 */
	...
	protected int[] checkCollision(String direction, int dx, int dy) {
		...
		Figure[][] map = this.gameMap.getMap(); //map est un tableau à 2 dimensions
		...
		for (int i = xCoords - 1; i <= xCoords + 1; i++) {
			for (int j = yCoords - 1; j <= yCoords + 1; j++) {
				Figure f = map[j][i]; //on analyse chaque case de map (logique)
				if (this.checkCollisionWithFigure(f, dx, dy)) {
					if (f instanceof Wall) { // la case est une instance de Wall
					...
					} else if (f instanceof Food) { // la case est une instance de Food (petite/grosse boule)
	...
	/**Renvoie les coordonnées sur la map de l'instance de Créature
	 * 
	 * @return ret = { colonne, ligne }
	 */
